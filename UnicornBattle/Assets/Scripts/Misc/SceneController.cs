    ...
    public class SceneController : Singleton<SceneController>
        {
            ...
            {
                return RequestSceneChange(GameScenes.Profile, .333f);
            }
            ...
            public bool RequestSceneChange(GameScenes nextScene, float delay = 0)
            {
                if (this.sceneChangeInProgress == null)
                {
                    ...
                    string scene = scenes.FirstOrDefault(z => z == match);
                    ...
                    sceneChangeInProgress = StartCoroutine(ChangeScene(nextScene, delay));
            ...
            IEnumerator ChangeScene(GameScenes nextScene, float delay)
            {
                ...
                var operation = SceneManager.LoadSceneAsync(nextScene.ToString());
                while (!operation.isDone)
                {
                    yield return null;
                }
                TelemetryManager.RecordScreenViewed(GetTelemetryScreenId(nextScene));
            ...
            public static TelemetryScreenId GetTelemetryScreenId(GameScenes scene)
            {
                switch (scene)
                {
                    case GameScenes.Null:
                        return TelemetryScreenId.Untracked;
                    case GameScenes.Authenticate:
                        return TelemetryScreenId.Untracked;
                    case GameScenes.CharacterSelect:
                        return TelemetryScreenId.Character;
                    case GameScenes.Profile:
                        return TelemetryScreenId.Main;
